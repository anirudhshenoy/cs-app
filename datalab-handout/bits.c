...
int isAsciiDigit(int x) {
  int upperBound = ~(0x39) + x;
  int lowerBound = ~(0x30) + 1 + x;
  lowerBound = ((1<<31) & lowerBound) >> 31;
  upperBound = ((1<<31) & upperBound) >> 31; 
...
int isLessOrEqual(int x, int y) {
  int xsign = (x >> 31);
  int ysign = (y >> 31);
  int diffSign = (y + (~x) +1)>>31;
  return ((xsign ^ ysign) & (xsign & 1)) | ((!(xsign ^ ysign)) & !diffSign);
